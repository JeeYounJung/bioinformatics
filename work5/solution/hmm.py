...
class HiddenMarkovModel:
    ...
    def estimate_parameters(self, training_data, emission_pseudocount=0, transition_pseudocount=0):
        ...
        Args:
            ...
            emission_pseudocount: a pseudocount to add to each observed emission count 
                when computing the parameter values.  The default is zero, which corresponds
                to maximimum likelihood estimates without smoothing.  A value of one for
                this parameter corresponds to Laplace smoothing.
            transition_pseudocount: pseudocount for transition probabilities
        """
        # initialize matrices of counts
        transition_count_matrix = matrix(len(self.states), len(self.states), transition_pseudocount)
        initial_counts = [transition_pseudocount] * len(self.states)
        emission_count_matrix = matrix(len(self.states), len(self.chars), emission_pseudocount)
