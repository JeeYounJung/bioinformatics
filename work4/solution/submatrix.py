...
def print_row(fields, width=5):
    print("".join(["{:>{width}}".format(field, width=width) for field in fields]))

def print_vector(v, width=5):
    print_row(v.keys(), width=width)
    print_row(v.values(), width=width)
...
def print_matrix(m, width=5):
    ...
    print_row([""] + labels)
    for a in labels:
        print_row([a] + [m.get((a, b), "") for b in labels], width=width)
